...
get_proton_clang-13.0() {
	...
	TC="$TOOLCHAIN_DIR/clang-r450784e"
	...
	if [[ $DISABLE_LLD == "1" ]]; then
		MAKEOPTS="CC=clang AR=${PFX_OVERRIDE}llvm-ar AS=${PFX_OVERRIDE}llvm-as NM=${PFX_OVERRIDE}llvm-nm STRIP=${PFX_OVERRIDE}llvm-strip
					OBJCOPY=${PFX_OVERRIDE}llvm-objcopy OBJDUMP=${PFX_OVERRIDE}llvm-objdump READELF=${PFX_OVERRIDE}llvm-readelf
					HOSTAR=${PFX_OVERRIDE}llvm-ar HOSTAS=${PFX_OVERRIDE}llvm-as LLVM=1 LLVM_IAS=1"
	else
		if [[ $DISABLE_IAS == "1" ]]; then
...
build() {
	...
	MAKEOPTS="CONFIG_SECTION_MISMATCH_WARN_ONLY=y $MAKEOPTS"
...
build_zip() {
	...
	properties() { '
	kernel.string=WildAngelKernel v2.0 battery by WildAngelDev
	device.name1=aristotle
	do.devicecheck=1
	do.modules=0
	do.systemless=0
	do.cleanup=1
